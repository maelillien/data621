...
model_data2 <- cbind(us_production,
      ...
      tmin %>% filter(!YEAR %in% c(2012,2013)) %>% replace(is.na(.), 0) %>% select(-c(YEAR,PRODUCTION))
      )

#model_data2 <- model_data2 %>% mutate(across(where(is.logical), function (x) {as.factor(x*1)}))
